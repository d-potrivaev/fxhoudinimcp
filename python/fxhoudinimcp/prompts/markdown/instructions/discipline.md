...
6.  EXPOSE THE KNOBS THAT EARN IT. When an HDA or subnet holds the setup, its own parameter interface carries the knobs (promote them there), never a CTRL null inside it. A CTRL null with spare parameters, channel-referenced into the network (`ch("../CTRL/...")`), is only for a loose network where one value drives several nodes, or when the user asks. A simple setup (a terrain with scattered copies) needs neither: set the values on the nodes.
